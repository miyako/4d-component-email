...
* SMTP_QuickSend（MIMEモード）を使用する場合の注意点

``From``および``To``ヘッダーは，プラグインコマンドによって自動的に追加されますので，コンポーネントでは定義しないでください。一部のサービス（例：Google）は，複数の``From``が含まれるメッセージの配信を拒否するようです。

4Dネイティブコマンドの``BASE64 ENCODE``は，特に改行コードを挿入することなく，エンコードされた文字列を返します。一部のサービスは，76文字あるいは998文字を超過するメッセージの配信を拒否するようです。下記のプラグインコマンドは，RFCに準拠するような仕方でBASE64文字列に改行を挿入します。
